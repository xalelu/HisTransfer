...
Public Class HisTransplant
    ...
    'Public endCut As Integer = _tempRecordTransFile
    ...
    Public Shared _context As DataType '紀錄當下的處理的資料類型(His 或 Alarm)

    Public Shared CheckHisFile As Dictionary(Of String, List(Of String))
    Public Shared CheckAlarmFile As List(Of String)
    ...
    Public Sub startMove(DT As DataType)
        Select Case DT
            Case DataType.His
                ...
                HisTransplant._context = DataType.His
            Case DataType.Alarm
                Dim nThread_Agent_Init As New Threading.Thread(AddressOf AlarmTransplant_start)
                nThread_Agent_Init.IsBackground = True
                nThread_Agent_Init.Start()
                HisTransplant._context = DataType.Alarm
        End Select
    End Sub

    Public Function check(_type As DataType)
        Select Case _type
            '檢查"歷史即時值"是否須要移植
            Case DataType.His
            ...
            '檢查"警報"是否須要移植
            Case DataType.Alarm
                Dim _directory As String = Environment.CurrentDirectory & "\Alr" '"C:\Users\JNC\Desktop\測試" 'Console.ReadLine()
                Dim access_file_count As Integer = Directory.GetFiles(_directory & "\*.mdb").Length
                Dim sqlite_file_count As Integer = Directory.GetFiles(_directory & "\*.db3").Length
                Return sqlite_file_count Mod (access_file_count + sqlite_file_count)
        End Select
    ...
    Public Sub Transplant_start()
        Try
            ...
            If Not Directory.Exists(Mydirectory) Then
                RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss") & "Access to Sqlite移植作業: 抱歉, Input目錄不存在..")
                Exit Sub
            Else
                RaiseEvent NodeStatusChanged("開始進行 歷史即時資料 備份 " & Mydirectory & " 至 " & Mydestination & " ...")
            End If

            Directory.CreateDirectory(Mydirectory & "\PAST_HIS") '將過去的Access, 在移轉完後, 移動到PAST_HIS位置
            '*********** 測試下方函式用 20200124 ************
            Dim DeviceMac As List(Of DirectoryInfo) = GetAccessDirectory(Mydirectory) '取得His下的目錄資訊.(各裝置的目錄)
            '*********** 上述三行, 可能不會再用到. ************

            Me.ReadTemp(DataType.His) '讀取上次備份一半的紀錄檔(紀錄在_tempTransSet變數裡)
            If _tempTransSet.Count = 0 Then '無可作業內容
                remove_tempRecordTransFile(DataType.His) '刪除快取
                RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") & ": His無可作業內容.")
                Thread.Sleep(1000) '暫停個1秒
                RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") & ": 開始進行Alarm移轉!")
                startMove(DataType.Alarm)
                Exit Sub
            End If
    ...
    Public Sub AlarmTransplant_start()
        Try
            ...
            If Not Directory.Exists(Mydirectory) Then
                RaiseEvent NodeStatusChanged("抱歉, 你輸入的目錄不存在..")
                ...
                RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss") & "Alarm Access to Sqlite移植作業: 抱歉, Input目錄不存在..")
            ...
            Else
                ...
                RaiseEvent NodeStatusChanged("開始進行 警報資料 備份 " & Mydirectory & " 至 " & Mydestination & " ...")
            ...
            Directory.CreateDirectory(Mydirectory & "\PAST_ALARM") '將過去的Access, 在移轉完後, 移動到PAST_ALARM位置
            '*********** 測試下方函式用 20201005 ************
            Dim AlarmFn As List(Of FileInfo) = GetAccessTB(New DirectoryInfo(Mydirectory), "*.mdb") '取得Alarm下的目錄資訊.(各裝置的目錄)
            '*********** 上述三行, 可能不會再用到. ************

            Me.ReadTemp(DataType.Alarm) '讀取上次備份一半的紀錄檔(紀錄在_tempTransSet 或 _tempAlarmSet變數裡)

            If _tempAlarmSet.Count = 0 Then '無可作業內容
                remove_tempRecordTransFile(DataType.Alarm)
                RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") & ": Alarm無可作業內容.")
                Thread.Sleep(3000) '暫停3秒, 即終止程式
            ...
            If _tempAlarmSet IsNot Nothing And _tempAlarmSet.Count > 0 Then
                Dim Index As Integer = 0
                Dim _coremapfile As New List(Of List(Of String)) '暫處理執行緒對應要處理的年月檔名稱

                For i As Integer = 0 To Core - 1 '視建立的執行緒數量, 加入對應的資料庫處理列表.
                    jobschedual.Add(New List(Of String))
                    _coremapfile.Add(New List(Of String))
                Next

                Console.WriteLine("check alarm1:" & _tempAlarmSet.Count)
                '將所屬的年月的檔案 分配到各個裝置處理核心

                For Each a As String In _tempAlarmSet.Keys
                    Dim sk As Integer = Index Mod Core
                    'jobschedual(sk).Add(a)
                    _coremapfile(sk).Add(a)
                    Index += 1
                Next
                Console.WriteLine("check alarm2:" & _coremapfile.Count) '按"年月"分配給各執行緒要處理的月份

                '比對原始資料夾裡的警報檔案(年月日) 與 紀錄檔的警報檔案(年月日)
                '由於是依年月為單位, 分配於不同執行緒進行處理, 為判斷原始檔案是否在紀錄檔中, 所以以年月來判斷是否要進行時間先後判斷
                For Each a As FileInfo In AlarmFn
                    Dim thetargetdate As String = a.Name.Substring(0, 6) 'a是自FileInfo來的資料檔列表.
                    For i = 0 To _coremapfile.Count - 1
                        If _coremapfile(i).Contains(thetargetdate) Then '查詢那個執行有包含該月份的.
                            '比對二者的時間
                            If Compare(a.Name, _tempAlarmSet(thetargetdate)) >= 0 Then
                                jobschedual(i).Add(a.Name)
                            End If
                        End If
                    Next
                Next

                Console.WriteLine("check alarm2: " & jobschedual.Count) '查詢整理後的列表
                '逐一啟動執行
                For i As Integer = 0 To Core - 1
        ...
        Catch ex As Exception
            Console.WriteLine("Exception_Main: " & ex.ToString())
            Environment.Exit(Environment.ExitCode) '若發生"多個線程式引發Exception, 則中斷跳出"
        Finally
            'Console.ReadKey()
        End Try
    End Sub
    ...
    Public Sub TransAlarmSqlite()
        ...
        Try
            If target.Count > 0 Then
                ...
                For i As Integer = 0 To target.Count - 1 'Alr目錄下的警報日期檔案
                    Dim origin As String = Mydirectory & "\" & target(i) '來源
                    Dim destin As String = origin.Replace(".mdb", ".db3") 'Mydestination & "\" & target(i) '目標

                    '查詢有無該DB3檔案, 無則建立之("yyyymmdd.db3")
                    If File.Exists(destin) Then
                        My.Computer.FileSystem.DeleteFile(destin,
                            Microsoft.VisualBasic.FileIO.UIOption.OnlyErrorDialogs,
                            Microsoft.VisualBasic.FileIO.RecycleOption.DeletePermanently,
                            Microsoft.VisualBasic.FileIO.UICancelOption.DoNothing)
                    End If
                    'If File.Exists(destin.Replace(".db3", ".db3-journal")) Then
                    If File.Exists(destin.Replace(".db3", ".db3-journal")) Then '新增多判斷.db3是否存在 20201118 sam add.
                        My.Computer.FileSystem.DeleteFile(destin.Replace(".db3", ".db3-journal"),
                            Microsoft.VisualBasic.FileIO.UIOption.OnlyErrorDialogs,
                            Microsoft.VisualBasic.FileIO.RecycleOption.DeletePermanently,
                            Microsoft.VisualBasic.FileIO.UICancelOption.DoNothing)
                        'End If
                    ...
                    Else
                        RaiseEvent NodeStatusChanged("建立資料表失敗: " & destin)
                    ...
                    yearmonth = target(i).Substring(0, 6)
                    'Dim _targetTemp As String = _tempAlarmSet(yearmonth)
                    '****-------------------------------------------------------------------20201005 sam
                    'For Each j As String In _tempAlarmSet.Keys '(依"年月"為鍵, 處理中的"年月日"為值)
                    Console.WriteLine("取數量: " & _tempAlarmSet.Count)

                    If _tempAlarmSet.ContainsKey(yearmonth) Then
                        Console.WriteLine("要跑的日期: " & target(i))
                        Console.WriteLine("紀錄的日期: " & _tempAlarmSet(yearmonth)) '

                        If Compare(target(i), _tempAlarmSet(yearmonth)) >= 0 Then 'j.Value
                            Console.WriteLine("*********要跑的日期比較大")
                            ...
                            For Each a As KeyValuePair(Of String, DataTable) In td
                                Select Case a.Key
                                    'StartTime, EndTime, AckTime這3個時間格式要改為yyyy-MM-dd HH:mm:ss
                                    Case "Acked"
                                        For Each b As DataRow In a.Value.Rows
                                            PPT &= "INSERT INTO Acked(""StartTime"", ""EndTime"", ""AckTime"", ""Value"", ""Label"", ""TagName"", ""Message"", ""Type"", ""Note"", ""Personnel"") VALUES(""" & getSqltieTime(b.Item(1).ToString()) & """, """ & getSqltieTime(b.Item(2).ToString()) & """, """ & getSqltieTime(b.Item(3).ToString()) & """, """ & b.Item(4).ToString() & """, """ & recombineLabelValue(macs, b.Item(5).ToString()) & """, """ & b.Item(6).ToString() & """, """ & b.Item(7).ToString() & """, """ & b.Item(8).ToString() & """, """ & b.Item(9).ToString() & """, """ & b.Item(2).ToString() & """);"
                                        Next
                                    Case "Alarm"
                                        For Each b As DataRow In a.Value.Rows
                                    ...
                                    Case "History"
                                        For Each b As DataRow In a.Value.Rows
                                            PPT &= "INSERT INTO History(""StartTime"", ""EndTime"", ""AckTime"", ""Value"", ""Label"", ""TagName"", ""Message"", ""Type"", ""Note"", ""Personnel"") VALUES(""" & getSqltieTime(b.Item(1).ToString()) & """, """ & getSqltieTime(b.Item(2).ToString()) & """, """ & getSqltieTime(b.Item(3).ToString()) & """, """ & b.Item(4).ToString() & """, """ & recombineLabelValue(macs, b.Item(5).ToString()) & """, """ & b.Item(6).ToString() & """, """ & b.Item(7).ToString() & """, """ & b.Item(8).ToString() & """, """ & b.Item(9).ToString() & """, """ & b.Item(2).ToString() & """);"
                                        Next
                                End Select
                            Next
                            ...
                            Console.WriteLine("checkthat: " & Mydestination & "\" & target(i))
                            RecordHandler(yearmonth, Mydestination & "\" & target(i), TransAction.Add, DataType.Alarm) '紀錄即將要移植的access檔
                            conn.Open()
                            RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy/MM/dd HH:m:ss") & " (" & Thread.CurrentThread.ManagedThreadId.ToString() & "): 進行Sqlite(Alarm)寫入處理(" & target(i) & ")")
                            Using tran As SQLiteTransaction = conn.BeginTransaction()
                                Try
                                    ...
                                    tran.Commit()
                                    '將Access檔案 移置到 Environment.CurrentDirectory & "\Alr" & "\PAST_ALARM"位置
                                    MoveFile(DataType.Alarm, origin)
                                    RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy/MM/dd HH:m:ss") & " (" & Thread.CurrentThread.ManagedThreadId.ToString() & "): 已完成Sqlite(Alarm)寫入處理(" & target(i) & ")")
                                    RaiseEvent NodeStatusChanged("Alarm檔案名稱: " & (Mydirectory & "\" & target(i)))
                                    'FileSystem.Rename(Mydirectory & "\" & target(i), GetNewFileName((Mydirectory & "\" & target(i))))
                                Catch ex As Exception
                    ...
                    End If
                Next
                RecordHandler(yearmonth, Nothing, TransAction.Remove, DataType.Alarm) '移除已完成的檔案紀錄
                '****-------------------------------------------------------------------20201005 sam
                'Next
            Else
                Console.WriteLine("the value is nothing..")
        ...
        Finally
            ...
            RaiseEvent NodeStatusChanged("結束" & Thread.CurrentThread.Name & "執行緒: ")
            ...
            If CheckEnd() Then '若為True, 代表所有執行isAlive = false
                ...
                RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") & ": Alarm移植作業結束.")
                'CreateHandlerLogFile(ReadContent.HandlerLog) '建立完成作業的旗標檔案(棄用)
                'Form1.LogBox.AppendText("移植作業結束." & Environment.NewLine)
                Thread.Sleep(5000) '暫停5秒, 即終止程式
                Environment.Exit(Environment.ExitCode) '強制終止程序
            End If
        End Try
    End Sub
    ...
    '將舊Access 移置到 對應的位置.
    Private Sub MoveFile(_type As DataType, filePath As String)
        Try
            Select Case _type
                Case DataType.His
                    filePath = filePath.Replace(".db3", ".mdb")
                    'Dim fileStepPaths As String() = filePath.Split("\")
                    Dim fileStepPaths As FileInfo = New FileInfo(filePath)
                    Dim newFileNewPath As String = Environment.CurrentDirectory & "\His\PAST_HIS"
                    System.IO.Directory.CreateDirectory(newFileNewPath)
                    'Dim newfilepath As String = newFileNewPath & "\" & (fileStepPaths(fileStepPaths.Length - 2) & "_" & fileStepPaths(fileStepPaths.Length - 1))
                    Dim newfilepath As String = newFileNewPath & "\" & (fileStepPaths.Directory.Name & "_" & fileStepPaths.Name)
                    My.Computer.FileSystem.MoveFile(filePath, newfilepath, True)
                Case DataType.Alarm
                    'Dim fileStepPaths As String() = filePath.Split("\")
                    Dim fileStepPaths As FileInfo = New FileInfo(filePath)
                    Dim newFileNewPath As String = Environment.CurrentDirectory & "\Alr\PAST_ALARM"
                    System.IO.Directory.CreateDirectory(newFileNewPath)
                    'Dim newfilepath As String = newFileNewPath & "\" & fileStepPaths(fileStepPaths.Length - 1)
                    Dim newfilepath As String = newFileNewPath & "\" & fileStepPaths.Name
                    My.Computer.FileSystem.MoveFile(filePath, newfilepath, True)
            End Select
        Catch ex As Exception
    ...
    Private Sub CreateHandlerLogFile(filepath As String)
        Dim fs As FileStream = File.Create(filepath)
        Dim info As Byte() = New UTF8Encoding(True).GetBytes(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") & " 完成移置作業!")
        fs.Write(info, 0, info.Length)
        fs.Close()
    End Sub

    '將access的時間 轉換為 yyyy-MM-dd HH:mm:ss
    Private Function getSqltieTime(accesstime As String)
        If accesstime <> "" Then
            Return DateTime.Parse(accesstime).ToString("yyyy-MM-dd HH:mm:ss")
        Else
            Return ""
        End If
    End Function
    ...
    Private Function getMacFormLabel(label As String, type As MathType)
        Select Case type
            Case MathType.quotient '取商數 [抓其對應的設備索引]
                Return (CType(label.Substring(5), Integer)) Mod 120
            Case MathType.remainder '取餘數 [重新指定頻道索引]
                Return (CType(label.Substring(5), Integer)) \ 120
        End Select
    End Function
    ...
    Public Sub ProcessCopy_transcation(a As DirectoryInfo, cretNewDirect As String)
        ...
        Try
            ...
            Else
                ...
                RaiseEvent NodeStatusChanged("閞始讀取Access資料")
                ...
                RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy/MM/dd HH:m:ss") & " (" & Thread.CurrentThread.ManagedThreadId.ToString() & "): 開始讀取Access資料")
                    ...
                    For Each b As FileInfo In _tableFile
                        ...
                        If Not File.Exists(db3file) Then
                            ...
                            Else
                                ...
                                RaiseEvent NodeStatusChanged("建立資料表失敗: " & db3file)
                ...
                RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy/MM/dd HH:m:ss") & " (" & Thread.CurrentThread.ManagedThreadId.ToString() & "): 結束讀取Access資料")
                ...
                'RaiseEvent NodeStatusChanged("閞始進行Sqlite批次寫入處理")
                ...
                RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy/MM/dd HH:m:ss") & " (" & Thread.CurrentThread.ManagedThreadId.ToString() & "): 進行Sqlite寫入處理")
                ...
                'RaiseEvent NodeStatusChanged("結束進行Sqlite批次寫入處理")
                ...
                RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy/MM/dd HH:m:ss") & " (" & Thread.CurrentThread.ManagedThreadId.ToString() & "): 完成Sqlite寫入處理")
        ...
        Finally
            'DebugList.Add("現在執行緒: " & Thread.CurrentThread.Name)
            RaiseEvent NodeStatusChanged("現在執行緒: " & Thread.CurrentThread.Name)
            'Form1.LogBox.AppendText("現在執行緒: " & Thread.CurrentThread.Name & Environment.NewLine)
            delThread(Thread.CurrentThread.Name) '刪除紀錄中的執行緒
            ...
            If CheckEnd() Then '若為True, 代表所有執行isAlive = false
                ...
                'DebugList.Add("移植作業結束.")
                RaiseEvent NodeStatusChanged("移植作業結束.")
                ''Form1.LogBox.AppendText("移植作業結束." & Environment.NewLine)
                Environment.Exit(Environment.ExitCode) '強制終止程序
            End If
        End Try
    ...
    Public Sub ProcessCopy_transcation2(a As DirectoryInfo, cretNewDirect As String)
            ...
            Else
                ...
                If _tableFile.Count > 0 Then
                    For Each b As FileInfo In _tableFile
                        ...
                        coreHandler(querylist)
                        querylist.Clear()
                        MoveFile(DataType.His, b.FullName) '將移轉成功的檔案, 移到另外一個位置
                        ...
                        'DebugList.Add(DateTime.Now.ToString("yyyy/MM/dd HH:m:ss") & " (" & Thread.CurrentThread.ManagedThreadId.ToString() & "): 完成Sqlite寫入處理")
                        RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy/MM/dd HH:m:ss") & " (" & Thread.CurrentThread.ManagedThreadId.ToString() & "): 完成Sqlite寫入處理")
                        'FileSystem.Rename(b.FullName, GetNewFileName(b.FullName))
                        'Form1.LogBox.AppendText(DateTime.Now.ToString("yyyy/MM/dd HH:m:ss") & " (" & Thread.CurrentThread.ManagedThreadId.ToString() & "): 完成Sqlite寫入處理" & Environment.NewLine)
                        'End If
                    Next
                    'DebugList.Add(" *********************** His移植前置作業完畢, 移轉中 *********************** ")
        ...
        Finally
            ...
            If CheckEnd() Then '若為True, 代表所有執行isAlive = false
                ...
                remove_tempRecordTransFile(DataType.His)
                'DebugList.Add("移植作業結束.")
                RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") & ": His移植作業結束.")
                'Form1.LogBox.AppendText("移植作業結束." & Environment.NewLine)
                'Environment.Exit(Environment.ExitCode) '強制終止程序
                Thread.Sleep(1000) '暫停個1秒
                If CheckAlarmFile.Count > 0 Then '如果警報資料
                    RaiseEvent NodeStatusChanged(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") & ": 開始進行Alarm移轉!")
                    startMove(DataType.Alarm)
                Else
    ...
    '取得要修改的檔案名稱 20201116 sam add
    Private Function GetNewFileName(filepath As String)
        Dim newFileName As String() = filepath.Split(".")
        Dim oldname As String = newFileName(0).Substring(newFileName(0).LastIndexOf("\")) '舊檔案名稱
        Dim newname As String = "_" & oldname
        newFileName(0).Replace(oldname, newname)
        Return newFileName(0) & newFileName(1)
    End Function
                ...
                For Each i As KeyValuePair(Of String, List(Of String)) In alldta
                    ...
                    Using tran As SQLiteTransaction = conn.BeginTransaction()
                        Try
                            ...
                            '    Dim result As Integer = cmd.ExecuteNonQuery() '若是寫入或更新的話, 即會回傳寫入更新的筆數.
                            'Next
                            RaiseEvent NodeStatusChanged(">>> " & DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") & ": 進行 (" & i.Key & " ) Sqlite的備份")
                            '(2)寫法2 [有rollback的情況發生]
                            ...
                            RaiseEvent NodeStatusChanged("<<< " & DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") & ": 已完成 (" & i.Key & " ) Sqlite的備份")
                        Catch ex As Exception
                            tran.Rollback() '執行失敗, 則RollBack.
                        End Try
                    End Using
    ...
    Public Sub ProcessCopy(a As DirectoryInfo, cretNewDirect As String)
        ...
        Else
            ...
            If tableFile.Count > 0 Then
                ...
                Next
                'DebugList.Add(" *********************** END *********************** ")
                RaiseEvent NodeStatusChanged(" *********************** END *********************** ")
        ...
        If CheckEnd() Then '若為True, 代表所有執行isAlive = false
            ...
            RaiseEvent NodeStatusChanged("移植作業完畢.")
    ...
    Public Shared _tempRecordTransFile As String = Environment.CurrentDirectory & "\_tempRecordTransFile.tmp" '歷史即時值
    Public Shared _tempAlarmTransFile As String = Environment.CurrentDirectory & "\_tempAlarmTransFile.tmp" '警報
    '(2) 紀錄運行中的紀錄集合處理
    ...
    'End Sub
    Private Shared _sLock As Object = {}
    '(3) 塞入(His)紀錄
    Public Sub RecordHandler(mac As String, dbfile As String, actiontype As TransAction, DbType As DataType)
        SyncLock HisTransplant._sLock
            remove_tempRecordTransFile(IIf(DbType = DataType.His, DataType.His, DataType.Alarm)) '刪除檔案, 再新建一個
                ...
                Case TransAction.Add
                    Select Case DbType
                        Case DataType.His
                            ...
                            End If
                        Case DataType.Alarm
                            If _tempAlarmSet.ContainsKey(mac) Then
    ...
    Public Sub recordNowAccessFile(type As DataType) '參數1 為access資料庫的絶對路徑.
        ...
        Try
            SyncLock _SyncLock
                ...
                Else
                    fs.Close()
                    File.Delete(IIf(type = DataType.His, _tempRecordTransFile, _tempAlarmTransFile))
                End If
            End SyncLock
        Catch ex As Exception
            'DebugList.Add("移植備份 紀錄功能發生例外錯誤: " & ex.Message)
            RaiseEvent NodeStatusChanged("移植備份 紀錄功能發生例外錯誤: " & ex.Message)
                ...
                Case DataType.His
                    ...
                    If HisInitDta.Count > 0 Then
                        For Each i As KeyValuePair(Of String, String) In HisInitDta
                            RecordHandler(i.Key, Environment.CurrentDirectory & "\His\" & i.Key & "\" & i.Value & ".mdb", TransAction.Add, type)
                        Next
                    Else
                        Exit Sub
                    End If
                Case DataType.Alarm
                    ...
                    If AlarmInitDta.Count > 0 Then
                        For Each i As KeyValuePair(Of String, String) In AlarmInitDta
                            RecordHandler(i.Key, Environment.CurrentDirectory & "\Alr\" & i.Value & ".mdb", TransAction.Add, type)
                        Next
                    Else
                        Exit Sub
                    End If
        ...
        Try
            ...
            Select Case type
                ...
                Case DataType.Alarm 'FileName: _tempAlarmTransFile.tmp
                    ...
                    If AlarmDirect IsNot Nothing And AlarmDirect.GetFiles.Length > 1 Then
    ...
    Public Sub remove_tempRecordTransFile(type As DataType)
        ...
        Catch ex As Exception
            ...
            RaiseEvent NodeStatusChanged("刪除移植紀錄發生錯誤: " & ex.Message)
    ...
    Public Function GenernalCreateTableAlarmSql(tbfile As String, Optional tbname As String = "Acked") As Boolean
        Dim Acked_QUERY As String = "CREATE TABLE Acked (No Integer PRIMARY KEY, StartTime DateTime, EndTime DateTime, AckTime DateTime, Value VARCHAR(10), Label VARCHAR(30), TagName VARCHAR(24), Message VARCHAR(30), Type VARCHAR(10), Note VARCHAR(24), Personnel VARCHAR(24), HH VARCHAR(10), HI VARCHAR(10), LL VARCHAR(10), LO VARCHAR(10), Deadband VARCHAR(5), DELAY VARCHAR(5));"
        Dim Alarm_QUERY As String = "CREATE TABLE Alarm (No Integer PRIMARY KEY, StartTime DateTime, EndTime DateTime, AckTime DateTime, Value VARCHAR(10), Label VARCHAR(30), TagName VARCHAR(24), Message VARCHAR(30), Type VARCHAR(10), Note VARCHAR(24), Personnel VARCHAR(24), HH VARCHAR(10), HI VARCHAR(10), LL VARCHAR(10), LO VARCHAR(10), Deadband VARCHAR(5), DELAY VARCHAR(5));"
        Dim History_QUERY As String = "CREATE TABLE History (No Integer PRIMARY KEY, StartTime DateTime, EndTime DateTime, AckTime DateTime, Value VARCHAR(10), Label VARCHAR(30), TagName VARCHAR(24), Message VARCHAR(30), Type VARCHAR(10), Note VARCHAR(24), Personnel VARCHAR(24), HH VARCHAR(10), HI VARCHAR(10), LL VARCHAR(10), LO VARCHAR(10), Deadband VARCHAR(5), DELAY VARCHAR(5));"
        Dim querylist As List(Of String) = New List(Of String)()
